...
contract TestDeployer is MetadataDeployment {
    IERC20 constant USDC = IERC20(0xB833E8137FEDf80de7E908dc6fea43a029142F20);
    IWETH constant WETH_MAINNET = IWETH(0x9Dc08C6e2BF0F1eeD1E00670f80Df39145529F81); //use WTAO for WETH
        ...
        returns (DeploymentResultMainnet memory result)
    {
        ...
        PythAggregatorV3 WTAOOracle = new PythAggregatorV3(
                PYTH_ORACLE, // Pyth contract address
                WTAO_PRICE_FEED_ID // TAO price ID
        
        );

        result.externalAddresses.ETHOracle = address(WTAOOracle);
        result.externalAddresses.RETHOracle = address(WTAOOracle);
        result.externalAddresses.STETHOracle = address(WTAOOracle);
        result.externalAddresses.WSTETHToken = address(WTAOOracle);

        result.externalAddresses.RETHToken = VTAO_ADDRESS;
        ...
        // WETH
        IWETH WETH = IWETH(0x9Dc08C6e2BF0F1eeD1E00670f80Df39145529F81); //use WTAO for WETH
        ...
        // VTAO
        vars.collaterals[1] = IERC20Metadata(VTAO_ADDRESS);
        ...
        ICurveStableswapNGPool usdcCurvePool = ICurveStableswapNGPool(address(0));
