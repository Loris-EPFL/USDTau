...
import {UniswapFlashLoan} from "src/Zappers/Modules/FlashLoans/UniswapFlashLoan.sol";
...
import {IWTAO} from "src/Interfaces/IWTAO.sol";

//TAO Specific

// Price feeds
import {xTokenPriceFeed} from "src/PriceFeeds/xTokenPriceFeed.sol";
...
contract DeployLiquity2Script is DeployGovernance, UniPriceConverter, StdCheats, MetadataDeployment, Logging {
    ...
    address payable VTAO_ADDRESS = payable(0x31049849CCdD983D8F401077537438eaF12B12Af);
    // address constant WTAO_ADDRESS = 0x9Dc08C6e2BF0F1eeD1E00670f80Df39145529F81; 
    address constant WTAO_ADDRESS = 0x5F3B70e0c089a1e3020B1990823Bc241a7bF3522; //WTAO from Chainlink registry
    

    address WETH_ADDRESS = WTAO_ADDRESS;
    address USDC_ADDRESS = 0xB833E8137FEDf80de7E908dc6fea43a029142F20; //Taofi Hyperlane USDc
    ...
    IWETH WETH;
    IERC20Metadata USDC;
    address WSTETH_ADDRESS = VTAO_ADDRESS;
    // address RETH_ADDRESS = 0xae78736Cd615f374D3085123A210448E74Fc6393;
    address ETH_ORACLE_ADDRESS = address(0);
    address X_TOKEN_ADDRESS = 0x0Aea63F0f4A1DcC1d6641Bf393A0032c2C33f88B; //xSN62 TOken address
    uint16 X_TOKEN_NETUID = 62;
    // address RETH_ORACLE_ADDRESS = address(0);
    address STETH_ORACLE_ADDRESS = address(VTAO_ADDRESS); //Put Null address for vTAO oracle so it switches to canonical rate instead (TAO * vTAOtoTAO rate)
    ...
    // Sepolia
    ISwapRouter constant uniV3RouterSepolia = ISwapRouter(0x667A1AA098D03f788eBaD7678B7c02504EaC6092); //Replaced with TaoFI router 0x667A1AA098D03f788eBaD7678B7c02504EaC6092 
    IQuoterV2 constant uniV3QuoterSepolia = IQuoterV2(0xd7D7D7075139CE8CE02f97Bd113f3f89f60851C3); //From TaoFi quoter : 0xd7D7D7075139CE8CE02f97Bd113f3f89f60851C3
    IUniswapV3Factory constant uniswapV3FactorySepolia = IUniswapV3Factory(0x20D0Cdf9004bf56BCa52A25C9288AAd0EbB97D59); //Using TaoFi Factory : 0x20D0Cdf9004bf56BCa52A25C9288AAd0EbB97D59
    INonfungiblePositionManager constant uniV3PositionManagerSepolia =
        INonfungiblePositionManager(0x61EeA4770d7E15e7036f8632f4bcB33AF1Af1e25); //From Taofi UNIV3Pos Manager : 0x61EeA4770d7E15e7036f8632f4bcB33AF1Af1e25
    // Mainnet
    ISwapRouter constant uniV3RouterMainnet = ISwapRouter(0x667A1AA098D03f788eBaD7678B7c02504EaC6092);
    IQuoterV2 constant uniV3QuoterMainnet = IQuoterV2(0xd7D7D7075139CE8CE02f97Bd113f3f89f60851C3);
    IUniswapV3Factory constant uniswapV3FactoryMainnet = IUniswapV3Factory(0x20D0Cdf9004bf56BCa52A25C9288AAd0EbB97D59);
    INonfungiblePositionManager constant uniV3PositionManagerMainnet =
        INonfungiblePositionManager(0x61EeA4770d7E15e7036f8632f4bcB33AF1Af1e25);
    address uniswapV3FactoryAddress = 0x20D0Cdf9004bf56BCa52A25C9288AAd0EbB97D59;

    address uniV3WTAOUSDCPool = 0x6647dcbeb030dc8E227D8B1A2Cb6A49F3C887E3c; //TAOFI WTAO-USDC Pool on Uniswap V3
    address univ3WTAOBoldPool;
    ...
    function run() external {
        ...
        if (vm.envBytes("DEPLOYER").length == 20) {
            ...
            deployer = vm.envAddress("DEPLOYER");
                    console2.log("deployer qsd", deployer);
            ...
            deployer = vm.addr(privateKey);
                    console2.log("deployer sqd", deployer);

            vm.startBroadcast(privateKey);
        }
        console2.log("deployer afret", deployer);
        ...
        //TODO: Uncomment when deploying for Real
        ...
        //Subtensor Mainet ID = 964 
        if (block.chainid == 964) {
            // mainnet
            WETH = IWETH(WETH_ADDRESS);
            USDC = IERC20Metadata(USDC_ADDRESS);
            // curveStableswapFactory = curveStableswapFactoryMainnet;
            // SwapRouter uniswapv3Router = new SwapRouter(uniswapV3FactoryAddress, WETH_ADDRESS);
            uniV3Router = ISwapRouter(address(uniV3RouterMainnet));
        ...
        // Removed rETH since we only have 2 branches now
        ...
        _deployTestUniswapv3Pool(deployed);


        vm.stopBroadcast();

        // vm.writeFile("deployment-manifest.json", _getManifestJson(deployed, governanceManifest));
        vm.writeFile("deployment-manifest.json", _getManifestJson(deployed, "0x0000000000000000000000000000000000000000"));
    ...
    function _deployAndConnectContracts(
        ...
        // Collaterals
        if (block.chainid == 964 && !useTestnetPriceFeeds) {
            // mainnet
            // ETH
            ...
            // xToken
            vars.collaterals[2] = IERC20Metadata(X_TOKEN_ADDRESS);
        } else {
            // Sepolia
        ...
        // Deploy CollateralRegistry with upgradeable pattern
    ...
    function _deployTestUniswapv3Pool(DeploymentResult memory deployed) internal {
            //Mint vTAO
            // IvTAO(VTAO_ADDRESS).stake{value: 100 ether}(deployer);
       
            //TODO test to deploy Univ3 pool
            univ3WTAOBoldPool = uniswapV3FactoryMainnet.createPool(WTAO_ADDRESS, address(deployed.boldToken), UNIV3_FEE_WETH_COLL);

            //TODO Delete when prod.
            // _mintBold(100000 * 1e18, 400, deployed.contractsArray[0]); //mint 100k Bold for liquidity on Univ3
            uint256 boldAmount = 100000 * 10e18;
            IWTAO(WTAO_ADDRESS).deposit{value: 8000 ether}();
            (uint256 price,) = deployed.contractsArray[0].priceFeed.fetchPrice();
            console2.log("price", price);
            _mintBold(boldAmount, price, deployed.contractsArray[0]);
            console.log("balanceOf Bold", IERC20(address(deployed.boldToken)).balanceOf(deployer));

            address boldAddressPool = address(deployed.boldToken);
            IUniswapV3Pool(univ3WTAOBoldPool).initialize(priceToSqrtPriceX96(price)); //418 USD per TAo from TAoFI

            
            console.log("univ3WTAOBoldPool: ", univ3WTAOBoldPool);
            uint256 _price = 418;
            ProvideUniV3LiquidityVars memory vars;
            
            uint256 _token1Amount = 500 * 1e18; // 500 WTAO

            vars.token2Amount = _token1Amount * _price; // Calculate Bold tokens needed: 500 WTAO * 418 USD/WTAO = 209,000 * 1e18 Bold

        if (address(WTAO_ADDRESS) < address(boldAddressPool)) {
            vars.tokens[0] = WTAO_ADDRESS;
            vars.tokens[1] = boldAddressPool;
            ...
            // deal(boldAddressPool, deployer, vars.amounts[1]+ 1e30); //Deal BoldTokens for the Pool
            // deal(WTAO_ADDRESS, deployer, vars.amounts[0] + 1e30); //Deal WTAO for the Pool
        } else {
            vars.tokens[0] = address(boldAddressPool);
            vars.tokens[1] = address(WTAO_ADDRESS);
            vars.amounts[0] = vars.token2Amount;
            vars.amounts[1] = _token1Amount;
            vars.price = _price;
            // deal(boldAddressPool, deployer, vars.amounts[0] + 1e30); //Deal BoldTokens for the Pool
            // deal(WTAO_ADDRESS, deployer, vars.amounts[1] + 1e30); //Deal WTAO for the Pool
        }

        // mint and approve - approve larger amounts to ensure sufficient allowance
        IERC20(vars.tokens[0]).approve(address(uniV3PositionManagerMainnet), vars.amounts[0] + 1e30);
        IERC20(vars.tokens[1]).approve(address(uniV3PositionManagerMainnet), vars.amounts[1] + 1e30);

        // mint new position
        int24 TICK_SPACING = IUniswapV3Pool(univ3WTAOBoldPool).tickSpacing();
        ( /* uint256 finalSqrtPriceX96 */ , int24 tick,,,,,) = IUniswapV3Pool(univ3WTAOBoldPool).slot0();
        //console2.log(finalSqrtPriceX96, "finalSqrtPriceX96");
        
        // Use a symmetric tick range centered around current price for balanced liquidity provision
        int24 tickRange = 100; // Half-range on each side of current price
        vars.tickLower = ((tick - tickRange) / TICK_SPACING) * TICK_SPACING;
        vars.tickUpper = ((tick + tickRange) / TICK_SPACING) * TICK_SPACING;

        console2.log("amount 0 for token", vars.amounts[0], vars.tokens[0]);
        console2.log("amount 1 for token", vars.amounts[1], vars.tokens[1]);
        ...
        INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager.MintParams({
            ...
            amount0Min: 1, // 20% slippage tolerance
            amount1Min: 1, // 20% slippage tolerance
            recipient: deployer,
            deadline: block.timestamp + 600 minutes
        });
        

        uniV3PositionManagerMainnet.mint(params);

        console2.log("balanceOf Bold after mint Univ3", IERC20(boldAddressPool).balanceOf(deployer));
        console2.log("balanceOf WTAO after mint Univ3", IERC20(WTAO_ADDRESS).balanceOf(deployer));
    ...
    function _deployAndConnectCollateralContracts(
        ...
        (uint256 price, bool isStale) = xTokenPriceFeed(address(contracts.priceFeed)).fetchPrice();
        console.log("price of Oracle", price);
        ...
        console.log("setting addresses in addresses registry");
        contracts.addressesRegistry.setAddresses(addressVars);
        
        console.log("deploying borrower operations");
    ...
    function _deployPriceFeed(address _collTokenAddress, address _borroweOperationsAddress)
        ...
        returns (IPriceFeed)
    {
        console.log("deploying price feeds", block.chainid, !useTestnetPriceFeeds);
        if (block.chainid == 964 && !useTestnetPriceFeeds) {
            console.log("deploying pyth price feeds");
            //Deploy Chainlink wrapped interface with Pyth Oracle
            // deployedWTaoAggregator = new PythAggregatorV3(
            //     PYTH_ORACLE, // Pyth contract address
            //     WTAO_PRICE_FEED_ID // TAO price ID
            // );

            ETH_ORACLE_ADDRESS = address(0); ///Use chainlink wrapped oracle of WTAO as ETH oracle

            // mainnet
            // ETH
            if (_collTokenAddress == address(WETH)) {
                console.log("deploying weth price feed");
                ...
                // wstETH
                console.log("deploying wsteth price feed");
                return new xTokenPriceFeed(
                    ETH_ORACLE_ADDRESS,
                    ETH_ORACLE_ADDRESS,
                    WSTETH_ADDRESS,
            ...
            } else if (_collTokenAddress == X_TOKEN_ADDRESS) {
                        
               

                // xToken
                console.log("deploying xToken price feed", X_TOKEN_ADDRESS);
                return new xTokenPriceFeed(
                    ETH_ORACLE_ADDRESS,
                    ETH_ORACLE_ADDRESS,
                    X_TOKEN_ADDRESS,
        ...
        console.log("should not print for chain 964");
        // Sepolia
    ...
    function _deployZappers(
        ...
        IFlashLoanProvider flashLoanProvider = new UniswapFlashLoan(uniV3WTAOUSDCPool);

        IExchange univ3exchange = new UniV3Exchange(
            _collToken,
            _boldToken,
            UNIV3_FEE_WETH_COLL,
            uniV3Router
        ...
        leverageZapper = _deployHybridLeverageZapper(_addressesRegistry, flashLoanProvider, univ3exchange, lst);
    }
    ...
    function _mintBold(uint256 _boldAmount, uint256 _price, LiquityContracts memory _contracts) internal {
        ...
        console2.log("balanceOf WETH", WETH.balanceOf(deployer));
        ...
        returns (string memory)
    {
        ...
        return string.concat(
            ...
            string.concat(
                string.concat('"branches":[', StringFormatting.join(branches, ","), "],"),
