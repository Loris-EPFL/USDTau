...
import {IWTAO} from "src/Tokens/IWTAO.sol";
...
contract DeployLiquity2Script is DeployGovernance, UniPriceConverter, StdCheats, MetadataDeployment, Logging {
    ...
    uint256 constant NUM_BRANCHES = 2;
    ...
    function _deployTestUniswapv3Pool(DeploymentResult memory deployed) internal {
            ...
            IWTAO(WTAO_ADDRESS).deposit{value: 9000 ether}();
            (uint256 price,) = deployed.contractsArray[0].priceFeed.fetchPrice();
        ...
        console2.log("balanceOf Bold", IERC20(boldAddressPool).balanceOf(deployer));
        console2.log("balanceOf WTAO", IERC20(WTAO_ADDRESS).balanceOf(deployer));
        ...
        returns (IPriceFeed)
    {
        ...
        if (block.chainid == 964 && !useTestnetPriceFeeds) {
            ...
            ETH_ORACLE_ADDRESS = address(deployedWTaoAggregator); ///Use chainlink wrapped oracle of WTAO as ETH oracle
