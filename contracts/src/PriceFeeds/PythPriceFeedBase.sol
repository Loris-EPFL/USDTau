...
abstract contract PythPriceFeedBase is IMainnetPriceFeed {
    ...
    Oracle public taoUsdOracleData;

    IBorrowerOperations borrowerOperations;

    constructor(address _taoUsdOracleAddress, uint256 _taoUsdStalenessThreshold, address _borrowOperationsAddress) {
        // Store TAO-USD oracle
        taoUsdOracleData.aggregator = AggregatorV3Interface(_taoUsdOracleAddress);
        taoUsdOracleData.stalenessThreshold = _taoUsdStalenessThreshold;
        taoUsdOracleData.decimals = taoUsdOracleData.aggregator.decimals();
